...
from src.utils.distributed import run_on_main
from src.dataio.dataloader import LoopedLoader
from src.dataio.dataloader import SaveableDataLoader
# from speechbrain.dataio.sampler import DistributedSamplerWrapper
from src.dataio.sampler import ReproducibleRandomSampler,DistributedSamplerWrapper
...
def create_experiment_directory(
    ...
    try:
        ...
        if src.utils.distributed.if_main_process():
            ...
            src.utils.logger.setup_logging(log_config, logger_overrides)
            ...
            if save_env_desc:
                description_str = src.utils.logger.get_environment_description()
                with open(
        ...
        src.utils.distributed.ddp_barrier()
...
@src.utils.checkpoints.register_checkpoint_hooks
class Brain:
    ...
    * ``evaluate_batch()``
    ...
    def __init__(  # noqa: C901
        ...
        if total_params > 0:
            ...
            fmt_num = src.utils.logger.format_order_of_magnitude(total_params)
    ...
    def make_dataloader(
        ...
        **loader_kwargs : dict
        ...
        dataloader = src.dataio.dataloader.make_dataloader(
    ...
    def fit(
        ...
        * ``update_average()``
        ...
        for epoch in epoch_counter:
            ...
            enable = progressbar and src.utils.distributed.if_main_process()
            with tqdm(
                train_set,
                initial=self.step,
                dynamic_ncols=True,
    ...
    @src.utils.checkpoints.mark_as_saver
    def _save(self, path):
        save_dict = {
            "step": self.step,
    ...
    @src.utils.checkpoints.mark_as_loader
